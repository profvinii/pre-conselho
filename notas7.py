...
if merge:
    for a in recs.values():
        for pk,d in a['p'].items():
            ...
            a['p'][pk]=nd
# ---- imputação de 0,0 -------------------------------------------------
# Se a turma tem média lançada num componente e o estudante (que TEM notas no
# período) está sem essa média, a ausência entra como 0,0 e fica marcada.
porTurma=collections.defaultdict(list)
for (cod,n),a in recs.items(): porTurma[cod].append(a)
imput=0; impAlunos=set()
for cod,al in porTurma.items():
    for pk in ['1','2','3','4']:
        lanc={k for a in al if pk in a['p'] for k,v in a['p'][pk].items() if v[6] is not None}
        if not lanc: continue
        for a in al:
            d=a['p'].get(pk)
            if not d or not any(v[6] is not None for v in d.values()): continue
            for k in lanc:
                v=d.get(k)
                if v is None:
                    d[k]=[None,None,None,None,None,None,0.0,1]; imput+=1; impAlunos.add(a['t']+'|'+a['n'])
                elif v[6] is None:
                    v[6]=0.0
                    while len(v)<8: v.append(None)
                    v[7]=1; imput+=1; impAlunos.add(a['t']+'|'+a['n'])
print('imputações de 0,0: %d em %d estudantes'%(imput,len(impAlunos)))
...
out['imputacao']={'n':imput,'alunos':len(impAlunos),'regra':'0,0 quando a turma tem lançamento e o estudante não'}
